...
class BookingScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final workerState = ref.watch(workerProfileViewModelProvider(workerId));
    ...
    if (worker == null) {
      return Scaffold(
        ...
        appBar: AppBar(
          ...
          title: const Text('Book a Worker'),
        ),
        body: const Center(child: CircularProgressIndicator()),
      );
    }

    final args = BookingArgs(workerId: worker.id);
    final provider = bookingViewModelProvider(args);
    final state = ref.watch(provider);
    final vm = ref.read(provider.notifier);

    ref.listen<BookingState>(provider, (prev, next) {
    ...
    return AppBackScope(
      ...
      child: Scaffold(
        ...
        body: SafeArea(
          child: Column(
            children: [
              _StepperHeader(currentStep: state.currentStep, worker: worker),
              Expanded(
                child: IndexedStack(
              ...
              _BookingFooter(
                ...
                onSubmit: () async {
                  ...
                  if (outcome != BookingOutcome.success) return;
                  final requestId = ref.read(provider).createdRequest?.id;
                  if (requestId != null && requestId.isNotEmpty) {
                    context.pushReplacement(Routes.bookingSuccess(requestId));
                  }
                },
              ),
            ],
...
class _StepperHeader extends StatelessWidget {
  const _StepperHeader({required this.currentStep, required this.worker});
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(
        ...
        children: [
          Text(
            'Booking ${worker.name} · ${worker.primaryTrade}',
    ...
    return ListView(
      ...
      children: [
        AppTextField(
          ...
          hint: 'e.g. Living room AC makes a loud buzzing sound…',
        ...
        Text(
          'Helps the worker bring the right parts. Up to ${BookingViewModel.maxPhotos} photos.',
          style: AppTypography.bodySmall
              .copyWith(color: AppColors.textMuted),
        ),
        const SizedBox(height: 12),
  ...
  Widget build(BuildContext context) {
    return ListView(
      ...
      children: [
        ...
        Text(
          'Where should the worker show up? Pre-filled from your profile '
          '— edit if you\'re booking for somewhere else.',
          style: AppTypography.bodySmall
              .copyWith(color: AppColors.textMuted),
        ),
